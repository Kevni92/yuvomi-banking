...
function renderTransactionDetail(host, detail) {
  ...
  host.replaceChildren();
  host.insertAdjacentHTML('beforeend', `
    <div class="banking-transaction-detail-summary"><div><strong>${esc(String(title))}</strong><small>${esc(`${date} · ${status}`)}</small></div><strong class="banking-transactions-table__amount" data-direction="${esc(transaction.direction === 'outgoing' ? 'outgoing' : 'incoming')}">${esc(formatTransactionAmount(transaction))}</strong></div>
