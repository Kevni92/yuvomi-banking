...
test('opens only the Banking database and applies migrations idempotently', () => {
  ...
  try {
    ...
    assert.deepEqual(migrateDatabase(inMemory), [1, 2, 3, 4, 5, 6, 7, 8]);
    ...
    assert.deepEqual(appliedVersions.map((row) => Number(row.version)), [1, 2, 3, 4, 5, 6, 7, 8]);
    assert.equal(
      inMemory.prepare(
...
test('applies all later migrations to an existing phase-5 database', () => {
  ...
  assert.deepEqual(migrateDatabase(database), [6, 7, 8]);
